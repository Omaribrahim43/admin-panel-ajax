        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $.each(data, function (index, value) {
                    // Check if the value.image is empty or null
                    var imageSource = value.image ? 'uploads/' + value.image : 'images/default_profile.jpg';
                    table.row.add([
                        value.id,
                        value.first_name,
                        value.last_name,
                        '<img src="' + imageSource + '" style="width:50px;height:50px;border:2px solid gray;border-radius:8px;object-fit:cover">',
                        value.email,
                        value.role,
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.statusCode == 200) {
                    ...
                    $("#editForm")[0].reset();
                    $(".preview_img").attr("src", "images/default_profile.jpg"); // Default image source
                    $("#successToast").toast("show");
                    $("#successMsg").html(response.message);
                    fetchData();
                } else if (response.statusCode == 500) {
